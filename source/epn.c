				   ...
				   struct ast_vhub_req *req)
{
	...
	EPVDBG(ep, "ast_vhub_epn_kick_desc\n");
			      ...
			      gfp_t gfp_flags)
{
	...
	dev_info(&vhub->spi->dev, "ast_vhub_epn_queue\n");

	/* Paranoid checks */
	if (!u_req || !u_req->complete || !u_req->buf) {
		dev_warn(&vhub->spi->dev, "Bogus EPn request ! u_req=%p\n", u_req);
		if (u_req) {
			dev_warn(&vhub->spi->dev, "complete=%p internal=%d\n",
				     ...
				     bool restart_ep)
{
	...
	if (loops >= 1000)
		dev_warn(&ep->vhub->spi->dev, "Timeout waiting for DMA\n");
...
static int ast_vhub_epn_dequeue(struct usb_ep* u_ep, struct usb_request *u_req)
{
	...
	dev_info(&vhub->spi->dev, "ast_vhub_epn_dequeue\n");
...
struct ast_vhub_ep *ast_vhub_alloc_epn(struct ast_vhub_dev *d, u8 addr)
{
	...
	ep->buf = kzalloc(AST_VHUB_EPn_MAX_PACKET +
						  8 * AST_VHUB_DESCS_COUNT,
					  GFP_KERNEL);
	if (!ep->buf) {
		EPDBG(ep, "Couldn't allocate EPN\n");
		kfree(ep->ep.name);
		ep->ep.name = NULL;
		return NULL;
	}
	// ep->buf = dma_alloc_coherent(&vhub->spi->dev,
	// 			     AST_VHUB_EPn_MAX_PACKET +
	// 			     8 * AST_VHUB_DESCS_COUNT,
