...
#define DVDBG(d, fmt, ...)             \
	do                                 \
	{                                  \
		dev_info(&(d)->vhub->spi->dev, \
				 "%s " fmt, (d)->name, \
				 ##__VA_ARGS__);       \
	} while (0)

#define DVDBGONCE(d, fmt, ...)             \
	do                                     \
	{                                      \
		pr_crit_once(&(d)->vhub->spi->dev, \
					 "%s " fmt, (d)->name, \
					 ##__VA_ARGS__);       \
	} while (0)
...
#define UDCDBG(u, fmt...) dev_info(&(u)->spi->dev, fmt)
