...
static void ast_vhub_update_hub_ep1(struct ast_vhub *vhub,
				    unsigned int port)
{
	dev_info(&vhub->spi->dev, "port%d ast_vhub_update_hub_ep1. \n", port+1);
...
static void ast_vhub_wake_work(struct work_struct *work)
{
	...
	UDCDBG(vhub, "ast_vhub_wake_work\n");
...
ast_vhub_str_container_alloc(struct ast_vhub *vhub)
{
	...
	container = devm_kzalloc(&vhub->spi->dev, size, GFP_KERNEL);
	if (!container)
...
static int ast_vhub_init_desc(struct ast_vhub *vhub)
{
	...
	const struct device_node *vhub_np = vhub->spi->dev.of_node;
