					    ...
					    struct usb_ss_ep_comp_descriptor *ss)
{
	...
	DDBG(d, "ast_vhub_udc_match_ep %d\n", usb_endpoint_type(desc));
...
int ast_vhub_init_dev(struct ast_vhub *vhub, unsigned int idx)
{
	...
	struct device *parent = &vhub->spi->dev;
	int rc;
	...
	d->gadget.speed = USB_SPEED_UNKNOWN;
	d->gadget.dev.of_node = vhub->spi->dev.of_node;

	rc = usb_add_gadget_udc(d->port_dev, &d->gadget);
	if (rc != 0)
		goto fail_udc;
	d->registered = true;
	dev_info(d->port_dev, "Added gadget device at port:%s\n", dev_name(d->port_dev->parent));
	return 0;
 fail_udc:
