...
void ast_vhub_ep0_handle_setup(struct ast_vhub_ep *ep)
{
	...
	EPDBG(ep, "ast_vhub_ep0_handle_setup\n");
	...
	// set_address
	memmove(&crq, "\x00\x05\x01\x00\x00\x00\x00\x00", 8);
...
static void ast_vhub_ep0_rx_prime(struct ast_vhub_ep *ep)
{
	...
	dev_err(&vhub->spi->dev, "ast_vhub_ep0_queue.\n");
...
void ast_vhub_ep0_handle_ack(struct ast_vhub_ep *ep, bool in_ack)
{
	struct ast_vhub_req *req;
	struct ast_vhub *vhub = ep->vhub;
	struct device *dev = &vhub->spi->dev;
	bool stall = false;
	u32 stat;
...
static int ast_vhub_ep0_dequeue(struct usb_ep* u_ep, struct usb_request *u_req)
{
	...
	EPVDBG(ep, "ast_vhub_ep0_dequeue\n");
...
{
	// UDCVDBG(vhub, "ast_vhub_init_ep0\n");
	...
	if (dev) {
		// UDCVDBG(vhub, "ast_vhub_init_ep0 device\n");
		...
		// UDCVDBG(vhub, "ast_vhub_init_ep0 hub\n");
